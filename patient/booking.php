...
<body>
    ...
    if ($result && $result->num_rows > 0) {
        // Fetch the result only if there are rows
        $userfetch = $result->fetch_assoc();
        $userid = $userfetch["pid"];
        $username = $userfetch["pname"];
    } else {
        // Handle the case where no rows were returned or the query failed
        echo "No patient found with the provided email or the query failed.";
        exit();
    }
